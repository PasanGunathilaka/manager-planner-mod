...
- **Kind:** boundary
- **Verifies backlog item:** 3 (Task/WorkItem creation and viewing)
...
- **Kind:** boundary
- **Verifies backlog item:** 2 (Objective grouping and the planner grid)
...
- **Kind:** boundary
- **Verifies backlog item:** 7 (Progress notes and promise tracking)
...
- **Kind:** boundary
- **Verifies backlog item:** 8 (Accountability reporting)
...
- **Seam:** Pure function — `AccountabilityRow.Verdict`
- **Business rules pinned:** rule 7 — this is the exact nuance domain-model.md and
...
- **Business rules pinned:** rule 7 (the `<=` in `t.CompletedUtc.Value.Date <= promised.Date`)
...
- **Seam:** Stateful service boundary — `GetAccountabilityReportAsync`
- **Business rules pinned:** rule 7 (the strict `<` in `promised.Date < now.Date`)
...
- **Seam:** Stateful service boundary — `GetAccountabilityReportAsync`
- **Business rules pinned:** rule 7 ("only the most recently created promise note... is used —
...
- **Seam:** Stateful service boundary — `GetAccountabilityReportAsync`
- **Business rules pinned:** rule 7 (the `OrderByDescending(PromiseBroken).ThenByDescending(IsOverdue).ThenBy(Deadline ?? MaxValue)` sort, `PlanningService.cs:326-328`)
...
- **Seam:** Stateful service boundary — `ChangeStatusAsync`
- **Business rules pinned:** rule 9 — the CQ-018-flagged possibly-surprising half of this rule.
...
- **Kind:** boundary
- **Verifies backlog item:** 2 (Objective grouping and the planner grid — owner column)
...
- **Kind:** boundary
- **Verifies backlog item:** 10 (Project deletion, cascade)
...
- **Kind:** boundary
- **Verifies backlog item:** 9 (Task deletion, cascade)
...
- **Assert (shape):** Both the task and the note survive; `DiscoveredInMeetingId` and `MeetingId`
  are both now `null`.
- **Kind:** edge case
- **Verifies backlog item:** 6 (Meeting recording and history)
...
- **Kind:** boundary
- **Verifies backlog item:** 11 (Sample-data lifecycle)
...
1. **Project name required, ≤120 chars** — covered by GM-001.
2. **Task title required, ≤120 chars** — covered by GM-002.
3. **Objective title required, ≤150 chars** — covered by GM-003.
4. **Checklist label required, ≤300 chars** — covered by GM-004.
5. **Note text required, ≤2000 chars** — covered by GM-005.
6. **Note date backward/forward window** — covered by GM-006, GM-007.
7. **Promised-vs-delivered verdict computation (precedence order + sort)** — covered by GM-008
...
8. **Changing a task to its current status is a no-op** — covered by GM-019.
9. **Completion timestamp tracks the Done transition, both ways** — covered by GM-020 (into Done)
   and GM-021 (out of Done).
10. **Setting task owners replaces the full set, does not append** — covered by GM-022.
11. **Toggling a checklist item stamps/clears its completion time** — covered by GM-023.
